...
#ifdef __ANDROID__
// Android's libc++ only provides std::char_traits specializations for
...
// which libc++ rejects. Provide minimal specializations here so the
// Unicode string types below compile. These mirror the libstdc++
// generic int_type-based implementation closely enough for our usage
// (we mostly use them as buffers; no locale/collation ops).
#include <cstring>
namespace std {
template<typename T>
