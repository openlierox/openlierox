...
# --- 4. Patch the SDL2 manifest *template* --------------------------------
#
# In upstream commandergenius `project/app/src/main/AndroidManifest.xml`
# is a symlink to `project/AndroidManifest.xml`, which the port's
# changeAppSettings.sh overwrites from
# `project/jni/sdl2/android-project/app/src/main/AndroidManifest.xml`
# on every run. Patching the symlink target or its resolved file gets
# clobbered. Patch the template itself so the regenerated manifest has:
#
#   - android.permission.INTERNET + ACCESS_NETWORK_STATE
#     (OpenLieroX hawknl aborts at "Failed to open a socket for
#     networking" without it; AndroidAppSettings.cfg AccessInternet=y
#     is ignored in SDL2 mode because that knob only edits the
#     SDL 1.2 template).
#   - android:screenOrientation set from AndroidAppSettings.cfg
#     (the SDL2 template omits it, so phones respect rotation and
#     launch portrait — wrong for OpenLieroX).

SDL2_MANIFEST_TEMPLATE="$PORT_DIR/project/jni/sdl2/android-project/app/src/main/AndroidManifest.xml"
if [ -f "$SDL2_MANIFEST_TEMPLATE" ]; then
    if ! grep -q 'android.permission.INTERNET' "$SDL2_MANIFEST_TEMPLATE"; then
        echo ">>> adding INTERNET + ACCESS_NETWORK_STATE to SDL2 manifest template"
    ...
    if [ -n "$ORIENT" ] && ! grep -q 'android:screenOrientation=' "$SDL2_MANIFEST_TEMPLATE"; then
        echo ">>> injecting android:screenOrientation=\"$ORIENT\" into SDL2 manifest template"
        # The SDL2 template's activity is named SDLActivity; changeAppSettings
        # renames it to MainActivity when it regenerates. Inject on the
        # template's name to hit the template directly.
        sed -i "/<activity android:name=\"SDLActivity\"/a\\            android:screenOrientation=\"$ORIENT\"" \
            "$SDL2_MANIFEST_TEMPLATE"
    fi
fi
